...
class tsuspicion:
    ...
    def get_antibiotics(self, medication_table_in):
        ...
        medication_subset_filt.to_csv("all_patients_on_antibiotics.csv", sep=',', index=False)
        # medication_subset_filt["flag"] = medication_subset_filt["drugstopoffset"] - medication_subset_filt["drugstartoffset"]
        # medication_subset_filt["flag"] = medication_subset_filt["flag"] > 24*60
        # medication_subset_filt = medication_subset_filt[medication_subset_filt.flag == True]
        medication_columns = ['patientunitstayid', 'drugstartoffset']
        medication_subset_filt = medication_subset_filt[medication_columns]
        medication_subset_filt = medication_subset_filt.rename(columns={'drugstartoffset': 'max'})
        medication_subset_filt.to_csv("Tsuspicion_24hrLength_Antibiotics.csv", sep=',', index=False)
